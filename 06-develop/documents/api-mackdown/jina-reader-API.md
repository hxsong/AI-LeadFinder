...
通过企业名称抓取企业信息。Jina Reader 是一个网页内容解析 API，可以将任意网页转换为纯净的文本内容，用于后续的语义分析和信息提取。
...
### X-With-Links-Summary (Header)
- 类型：boolean
- 说明：是否包含链接摘要
- 默认值：false

### X-With-Images-Summary (Header)
- 类型：boolean
- 说明：是否包含图片摘要
- 默认值：false
...
|--------|------|------|
| Title | string | 页面标题 |
| URL Source | string | 源URL |
| Description | string | 页面描述（meta description） |
| 正文内容 | string | Markdown格式的页面正文 |
...
URL Source: https://www.abc-outdoor.com
Description: Leading distributor of premium hiking and outdoor equipment since 1995.
