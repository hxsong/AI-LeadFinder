# AI-LeadFinder 实体关系设计

> **最后更新**：2026-06-23（更新：Credits 统一替换为 MoliAI 点数）
...
| global_moli_ai_balance | int | 全局 MoliAI点数余额 | 5000 |
...
| moli_ai_used | int | 消耗 MoliAI点数 | 250 |
...
| moli_ai_consumed | int | 消耗 MoliAI点数 | 10 |
| created_at | datetime | 创建时间 | 2026-01-15 |
...
| analysis_report | text | 分析报告 | "..." |
| status | string | 线索状态 | new/contacted/qualified/converted |
| moli_ai_consumed | int | 消耗 MoliAI点数 | 5 |
| created_at | datetime | 创建时间 | 2026-01-15 |
...
### MoliAI点数流水 (MoliAITransaction)

**定义**：MoliAI点数消费记录，追踪每次 MoliAI点数变动。
...
| moli_ai_amount | int | MoliAI点数数量 | 2000 |
...
erDiagram
    Tenant ||--o{ Bot : "创建管理"
    Tenant ||--o{ User : "拥有"
    Tenant ||--o{ MoliAITransaction : "消费记录"
...
| Tenant → MoliAITransaction | 1:N | MoliAI点数流水记录 |
...
| 购买资源包 | 更新MoliAI点数余额、BOT席位 | Tenant、ResourcePackage |
| MoliAI点数消费 | 创建Transaction记录、更新余额 | MoliAITransaction、Tenant |
...
| 联系人挖掘 | 创建Contact记录、扣除MoliAI点数 | Contact、MoliAITransaction |
