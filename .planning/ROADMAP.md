...
| Plan | Description |
|------|-------------|
| P1.1 | 重写账号与资源管理模块（7个文件）：租户资源充值、资源下发确认、多BOT工作台、业务初始化、上传业务材料、独立站授权、AI业务分析中、业务分析报告 |
| P1.2 | 重写静态数据获客模块（9个文件）：搜客需求输入、搜客策略预览、搜客执行中、搜客记录、搜客结果列表、静态线索池、线索详情、业务分析报告（重复）、AI业务分析中（重复） |
| P1.3 | 重写动态流量获客模块（5个文件）：探针部署、访客雷达、访客行为详情、动态线索池、动态线索详情 |
| P1.4 | 重写全局设置模块（1个文件）：提示词语言管理 |
...
| DOC-02: 重写 21 个功能说明文档 | Phase 1 | Pending |
